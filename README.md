...
任何格式只需实现 [`RawDatasetAdapter`](./lerobot_dataconvert/adapters.py) 的三个方法。`inspect()` 返回的 `DatasetDescriptor.fields` 声明原始字段名称、shape、dtype、是否图像、分量名称和默认 LeRobot 目标字段；`iter_frames()` 在 `FrameSample.fields` 中提供对应值：
